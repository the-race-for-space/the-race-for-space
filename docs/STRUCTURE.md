...
- `KspIntegration/` — KSP API adapters, game events, ScenarioModule persistence hooks, vessel discovery, and stable celestial-body presentation ordering. `KspVesselDiscovery` resolves loaded and unloaded KSP vessel state into project-owned tracking snapshots before tracking rules consume it. Space Race funding presentation can request a Kerbin-relative orbital-distance sort key without hardcoding individual body names in the UI.
- `UI/` — race/funding presentation, Command Center visibility, and stock launcher interaction. UI code reads race state but does not own or advance gameplay progression. The Space Race view shows one selected funding contract using the existing detailed contract presentation, followed by collapsible `Available Now`, `Locked`, and `Expired` sections. Funding buttons are rendered four per row and sorted from Kerbin outward using each objective's existing celestial-body data, so newly added achievement or satellite funding targets enter the layout without a UI-specific body list. A small `?` button opens the in-game Help / Player Guide. Overview, Funding Targets, and Rival Agencies retain their existing responsibilities. Recurring IMGUI scratch buffers, layout options, and the window callback are reused rather than recreated for every draw event.
...
- `tests/TheRaceForSpace.ControllerTests/` compiles the real `SatelliteRaceController` against small test-only stand-ins for its existing KSP integration boundaries. It covers cross-module controller ordering without adding test seams to production code or requiring a KSP installation. The runner also hosts focused unlock-rule evaluator, UI-progress-helper, and rival-consumer regressions, including restoration of an overdue persisted funding boundary.
...
Live KSP API behavior such as loaded/unloaded vessel discovery, actual Career-funds integration, and Command Center layout still requires the documented in-game verification path.
