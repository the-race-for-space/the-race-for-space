...
- `Funding/` — nation/corporation sponsors, offers and awards. `PrototypeFundingCatalogue` owns the code-defined achievement rewards and satellite-network programme bootstrap for the current prototype. Achievement and satellite programmes now store the same `UnlockRuleDefinition` representation as milestone targets; current availability consumers move to the shared evaluator in Item 14C.
...
- `tests/TheRaceForSpace.Tests/` mirrors logic-heavy modules so milestone, simulation, tracking, funding, and persistence rules can be tested without requiring a live KSP scene. It now compiles the unlock definition model as part of the normal domain target/catalogue regression surface.
- `tests/TheRaceForSpace.ControllerTests/` compiles the real `SatelliteRaceController` against small test-only stand-ins for its existing KSP integration boundaries. It covers cross-module controller ordering without adding test seams to production code or requiring a KSP installation. The runner also hosts the KSP-independent Item 14A unlock-rule evaluator regressions because those source files are already dependencies of the future controller migration.
